                ...
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg bg-white p-12">
                    <form id="category-form" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-8 ">
                        ...
                        <div>
                            ...
                            <div class="mt-1">
                                <input type="text" name="event_name" id="event_name" autocomplete="given-name" class="text-black py-3 px-4 block w-full border-2  shadow-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                            </div>
                        </div>
...
<script>
        ...
        $(document).on('submit', '#category-form', function(e){
            ...
            $.ajax({
                ...
                url: "<?php echo route('admin.add.event.action')?>", // resource file with extension
                data:data,
                dataType:'JSON',
                processData: false,
            contentType: false,
