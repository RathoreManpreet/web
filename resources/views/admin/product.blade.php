...
<div class="pb-5 border-b border-gray-200 sm:flex sm:items-center sm:justify-between">
  ...
  <div class="mt-3 sm:mt-0 sm:ml-4 w-full text-right">
    <a href="{{route('admin.add.product')}}" type="button" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
      Add Product
    </a>
  </div>
</div>
        ...
        <table class="min-w-full divide-y divide-gray-200 table table-striped table-bordered" id="product-table"  style="width:100%">
          ...
          <tbody>
            ...
            <tr class="bg-white">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
              {{ strlen(strip_tags($product->product_desc)) > 50 ? substr(strip_tags($product->product_desc),0,50)."..." : strip_tags($product->product_desc) }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                <img src="{{asset($product->thumbnail)}}" class="w-8" alt="">
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
              
                @if($product->status == 0) 
                  <span class="text-red-600 font-semibold"> {{$product->getStatus($product->status)}} </span>
                @elseif($product->status == 1) 
                  <span class="text-green-600 font-semibold"> {{$product->getStatus($product->status)}} </span>
                @endif
              </td>
              
              <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                <a href="{{route('admin.edit.product',[$product->id])}}" class="text-indigo-600 hover:text-indigo-900 fas fa-edit"></a>
                <button data-url="{{route('admin.delete.product',[$product->id])}}" class="delete-product text-red-600 hover:text-red-900"><i class="fas fa-trash"></i></button>
              </td>
            </tr>
            @endforeach
        ...
        swal({
          ...
          if (willDelete) {
              ...
              $.ajax({
                  ...
                  dataType:'JSON',
                  success: function (e) {
                      swal("", "Product is successfully deleted!", "success")
                          .then((value) => {
