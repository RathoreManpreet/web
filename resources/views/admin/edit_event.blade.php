        ...
        <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
            <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg bg-white p-12">
                    <form id="category-form" method="POST" action="{{route('admin.update.event',[$event->id])}}" enctype="multipart/form-data" class="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-8 ">
                        @csrf
                        <div>
                            <label for="category_name" class="block text-lg font-medium text-gray-700">Event Name</label>
                            <div class="mt-1">
                                <input type="text" name="event_name" value="{{$event->event_name}}" id="event_name" autocomplete="given-name" class="text-black py-3 px-4 block w-full border-2  shadow-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                            </div>
                        </div>
                        ...
                        <div class="mt-1">
                            <textarea name="description" id="description">{{$event->event_desc}}</textarea>
                        </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex">
                                ...
                                <img src="{{asset($event->thumbnail)}}" class="flex-1 w-16" alt="">
                            </div>
                        </div>
                            ...
                            <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-5">
                                @foreach($event->eventImages as $image)
                                    <div class="bg-red-600 overflow-hidden shadow rounded-lg text-center">
                                        ...
                                        <button type="button" data-url="{{route('admin.delete.event.image',[$image->id])}}" class="delete-image inline-flex items-center p-1 border font-bold border-transparent rounded-full shadow-sm text-white">Delete</button>
                                    </div>
                                @endforeach
