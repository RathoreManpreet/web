...
<div class="pb-5 border-b border-gray-200 sm:flex sm:items-center sm:justify-between">
  ...
  <div class="mt-3 sm:mt-0 sm:ml-4 w-full text-right">
    <a href="{{route('admin.add.redx-specific')}}" type="button" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
      Add Redx Specific
    </a>
  </div>
</div>
  ...
  <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
      <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg bg-white p-4">
        <table class="min-w-full divide-y divide-gray-200 table table-striped table-bordered" id="product-table"  style="width:100%">
          ...
          <tbody>
            <!-- Odd row -->
            @foreach($redx_specifics as $redx_specific)
            <tr class="bg-white">
              ...
              {{ strlen(strip_tags($redx_specific->description)) > 50 ? substr(strip_tags($redx_specific->description),0,50)."..." : strip_tags($redx_specific->description) }}
              </td>
            
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
              
                @if($redx_specific->status == 0) 
                  <span class="text-red-600 font-semibold"> {{$redx_specific->getStatus($redx_specific->status)}} </span>
                @elseif($redx_specific->status == 1) 
                  <span class="text-green-600 font-semibold"> {{$redx_specific->getStatus($redx_specific->status)}} </span>
                @endif
              </td>
              
              <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                <a href="{{route('admin.edit.redx-specific',[$redx_specific->id])}}" class="text-indigo-600 hover:text-indigo-900 fas fa-edit"></a>
                <button data-url="{{route('admin.delete.redx-specific',[$redx_specific->id])}}" class="delete-product text-red-600 hover:text-red-900"><i class="fas fa-trash"></i></button>
              </td>
            </tr>
            @endforeach
        ...
        swal({
          ...
          if (willDelete) {
              ...
              $.ajax({
                  ...
                  dataType:'JSON',
                  success: function (e) {
                      swal("", "Data is successfully deleted!", "success")
                          .then((value) => {
