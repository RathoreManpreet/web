...
<div class="pb-5 border-b border-gray-200 sm:flex sm:items-center sm:justify-between">
  ...
  <div class="mt-3 sm:mt-0 sm:ml-4 w-full text-right">
    <a href="{{route('admin.add.category')}}" type="button" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
      Add Category
    </a>
  </div>
</div>
        ...
        <table class="min-w-full divide-y divide-gray-200 table table-striped table-bordered" id="product-table"  style="width:100%">
          ...
          @foreach($categories as $category)
            <tr class="bg-white">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ strlen(strip_tags($category->category_desc)) > 50 ? substr(strip_tags($category->category_desc),0,50)."..." : strip_tags($category->category_desc) }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
              
                @if($category->status == 0) 
                  <span class="text-red-600 font-semibold"> {{$category->getStatus($category->status)}} </span>
                @elseif($category->status == 1) 
                  <span class="text-green-600 font-semibold"> {{$category->getStatus($category->status)}} </span>
                @endif
              </td>
               
                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                <a href="{{route('admin.edit.category',[$category->id])}}" class="text-indigo-600 hover:text-indigo-900 fas fa-edit"></a>
                </td>
              </tr>
            @endforeach
